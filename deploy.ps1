...
ssh $Server "chmod -R 755 ${RemoteDir}/public/build && chown -R www-data:www-data ${RemoteDir}/public/build && docker cp ${RemoteDir}/public/build/. nemix-app-1:/var/www/html/public/build/ && docker exec nemix-app-1 chown -R www-data:www-data /var/www/html/public/build && docker exec nemix-app-1 chmod -R 755 /var/www/html/public/build"

Write-Host "==> [5/5] Running migrations, caching, and queue restart..." -ForegroundColor Cyan
ssh $Server "docker compose -f ${RemoteDir}/docker-compose.yml exec -T app php artisan migrate --force && docker compose -f ${RemoteDir}/docker-compose.yml exec -T app php artisan optimize:clear && docker compose -f ${RemoteDir}/docker-compose.yml exec -T app php artisan permission:cache-reset && docker compose -f ${RemoteDir}/docker-compose.yml exec -T app php artisan config:cache && docker compose -f ${RemoteDir}/docker-compose.yml exec -T app php artisan route:cache && docker compose -f ${RemoteDir}/docker-compose.yml exec -T app php artisan view:cache && docker compose -f ${RemoteDir}/docker-compose.yml exec -T app php artisan queue:restart"
