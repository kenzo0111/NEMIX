...
export default function RoleTable({
    ...
    return (
        <section
            ...
            className="bg-white dark:bg-slate-900 rounded-xl shadow-2xs border border-gray-200/90 dark:border-slate-800 overflow-hidden flex flex-col"
        >
            {/* Header with Search and Create Action */}
            <div className="p-4 sm:p-5 border-b border-gray-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900">
                <div className="min-w-0">
                    <h2
                        id="roles-heading"
                        className="text-base font-bold text-gray-900 dark:text-slate-100 truncate"
                    >
                        Configured Roles
                    </h2>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-3 w-full sm:w-auto">
                    <div className="relative w-full sm:w-64 min-w-0 flex-1 sm:flex-initial">
                        <input
                            ...
                            className="w-full pl-8 pr-7 py-2 text-xs border border-gray-300 dark:border-slate-700 rounded-lg focus:ring-1 focus:ring-red-900 dark:focus:ring-red-600 focus:border-red-900 dark:focus:border-red-600 bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100 placeholder:text-gray-400 dark:placeholder:text-slate-500 transition-all shadow-2xs"
                        />
                        ...
                        {searchQuery && (
                            <button
                                ...
                                className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-slate-300 text-xs font-bold cursor-pointer"
            ...
            <div className="overflow-x-auto flex-1 min-w-full">
                <table className="w-full text-left border-collapse min-w-[640px]">
                    <thead>
                        <tr className="border-b border-gray-200 dark:border-slate-800 bg-gray-50/90 dark:bg-slate-800/90 text-[11px] font-semibold tracking-wider text-gray-600 dark:text-slate-300 uppercase">
                    ...
                    <tbody className="divide-y divide-gray-200/80 dark:divide-slate-800 text-xs">
                        {roles.length > 0 ? (
                            roles.map((role) => {
                                const assignedCount =
                                ...
                                return (
                                    ...
                                    >
                                                    ...
                                                    <div className="flex items-center gap-2 mt-0.5 text-[11px] text-gray-400 dark:text-slate-500">
                                                        ...
                                                        {role.users_count !== undefined && (
                                                            <>
                                                                ...
                                                                <span className="inline-flex items-center gap-1 text-gray-500 dark:text-slate-400 font-medium">
                                        ...
                                        <td className="px-6 py-4">
                                            {isSystem ? (
                                                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-semibold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                                                    <Lock className="w-3 h-3 text-slate-500 dark:text-slate-400" />
                                                    System Role
                                                </span>
                                            ) : (
                                                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-semibold bg-gray-50 dark:bg-slate-800 text-gray-600 dark:text-slate-300 border border-gray-200 dark:border-slate-700">
                            ...
                            <tr>
                                <td colSpan={4} className="px-6 py-12 text-center text-gray-500 dark:text-slate-400">
                                    <div className="flex flex-col items-center gap-2">
                                        ...
                                        <p className="font-semibold text-gray-700 dark:text-slate-300 text-xs">
                                        ...
                                        {searchQuery && (
                                            <button
                                                type="button"
                                                onClick={() => onSearchChange('')}
                                                className="mt-2 px-3 py-1.5 text-xs font-semibold text-red-950 dark:text-red-300 hover:bg-red-50 dark:hover:bg-red-950/40 border border-red-900/30 dark:border-red-800/50 rounded-md transition-colors cursor-pointer"
                                            >
                                                Clear search query
                                            </button>
                                        )}
            ...
            {/* Footer count */}
            <div className="px-6 py-3.5 border-t border-gray-200 dark:border-slate-800 bg-gray-50/60 dark:bg-slate-900/60 flex items-center justify-between">
                <span className="text-xs text-gray-500 dark:text-slate-400 font-medium">
                    Showing <span className="font-semibold text-gray-800 dark:text-slate-200">{roles.length}</span> configured {roles.length === 1 ? 'role' : 'roles'}
                </span>
            </div>
        </section>
    );
