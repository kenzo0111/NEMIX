...
export const TransactionAuditToolbar: React.FC<TransactionAuditToolbarProps> = ({
    ...
    return (
        <div className="bg-white dark:bg-slate-900 rounded-lg border border-gray-200/90 dark:border-slate-800 shadow-2xs p-4 space-y-3 w-full min-w-0">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-gray-100 dark:border-slate-800">
                <div className="flex items-center gap-1.5 p-1 bg-gray-100/80 dark:bg-slate-800 rounded-md border border-gray-200/80 dark:border-slate-700 self-start sm:self-auto">
                    <button
                        type="button"
                        ...
                        className={`inline-flex items-center gap-1.5 px-3 py-1 text-xs font-semibold rounded transition-colors ${
                            viewMode === 'business'
                                ? 'bg-white dark:bg-slate-900 text-red-950 dark:text-red-300 shadow-2xs border border-gray-200/80 dark:border-slate-700 font-bold'
                                : 'text-gray-600 dark:text-slate-400 hover:text-gray-900 dark:hover:text-slate-200'
                        }`}
                        title="Show clean top-level business transactions with expandable details"
                    >
                        <Layers className="w-3.5 h-3.5 text-red-900 dark:text-red-400" />
                    ...
                    >
                        <ListFilter className="w-3.5 h-3.5 text-gray-500 dark:text-slate-400" />
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                ...
                <div className="relative flex-1 min-w-[240px]">
                    ...
                    <input
                        ...
                        className="w-full pl-9 pr-8 py-2 text-xs bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 text-gray-900 dark:text-slate-100 rounded-md focus:outline-none focus:ring-1 focus:ring-red-900 focus:border-red-900 font-medium placeholder-gray-400 dark:placeholder-slate-500"
                    />
                    {search && (
                        <button
                            type="button"
                            onClick={handleClearSearch}
                            className="absolute right-2.5 top-1/2 -translate-y-1/2 p-0.5 text-gray-400 dark:text-slate-400 hover:text-gray-600 dark:hover:text-slate-200 rounded-full hover:bg-gray-100 dark:hover:bg-slate-700"
                            title="Clear search"
                        >
                ...
                <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-2.5 w-full lg:w-auto">
                    ...
                    {hasActiveFilters && (
                        <button
                            type="button"
                            onClick={onReset}
                            className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-red-900 dark:text-red-300 bg-red-50 dark:bg-red-950/40 hover:bg-red-100 dark:hover:bg-red-950/60 border border-red-200 dark:border-red-900/40 rounded-md transition-colors"
            ...
            {showCustomDates && (
                <div className="pt-2 border-t border-gray-100 dark:border-slate-800 flex flex-wrap items-center gap-3 text-xs bg-gray-50/50 dark:bg-slate-800/50 p-2 rounded-md">
                    <div className="flex items-center gap-1.5 text-gray-600 dark:text-slate-400 font-medium">
                        <CalendarIcon className="w-3.5 h-3.5 text-gray-500 dark:text-slate-400" />
                        <span>Date Range:</span>
                    </div>

                    <div className="flex items-center gap-2">
                        <label className="text-gray-500 dark:text-slate-400 text-[11px] font-semibold">From:</label>
                        <input
                            type="date"
                            value={filters.date_from || ''}
                            onChange={(e) => handleCustomDateChange('from', e.target.value)}
                            className="px-2 py-1 text-xs border border-gray-300 dark:border-slate-700 rounded bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100 focus:ring-1 focus:ring-red-900 focus:border-red-900"
                        />
                    </div>

                    <div className="flex items-center gap-2">
                        <label className="text-gray-500 dark:text-slate-400 text-[11px] font-semibold">To:</label>
                        <input
                            type="date"
                    ...
                    {(filters.date_from || filters.date_to) && (
                        <button
                            ...
                            className="text-[11px] text-gray-500 dark:text-slate-400 hover:text-red-900 dark:hover:text-red-400 underline ml-auto"
                        >
                            Clear dates
                        </button>
                    )}
