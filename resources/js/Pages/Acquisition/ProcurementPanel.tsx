...
export default function ProcurementPanel({ auth, purchaseOrder }: { auth: any, purchaseOrder?: any }) {
    ...
    useEffect(() => {
        if (isEdit && purchaseOrder) {
            form.setData({
                ...purchaseOrder,
                items: purchaseOrder.items || [{ stock_no: '', unit: '', description: '', quantity: 0, unit_cost: 0, amount: 0, ris: false, par: false, ics: false, iar: false }],
            });
        } else if (!isEdit) {
            const now = new Date();
    ...
    const addItemRow = () => {
        form.setData('items', [
            ...form.data.items, 
            { stock_no: '', unit: '', description: '', quantity: 0, unit_cost: 0, amount: 0, ris: false, par: false, ics: false, iar: false }
        ]);
    };
                                ...
                                <div className="overflow-x-auto">
                                    <table className="min-w-full divide-y divide-gray-200">
                                        <thead className="bg-gray-50">
                                            <tr>
                                                <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase w-32">Stock No.</th>
                                                ...
                                                <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase w-48">Required Docs</th>
                                                ...
                                                <tr key={index}>
                                                    ...
                                                    <td className="px-2 py-2 align-top">
                                                        <div className="grid grid-cols-2 gap-2 text-[10px]">
                                                            <label className="flex items-center gap-1.5"><input type="checkbox" checked={item.ris} onChange={(e) => handleItemChange(index, 'ris', e.target.checked)} className="w-3.5 h-3.5 rounded text-red-600 focus:ring-0 border-gray-300"/> RIS</label>
                                                            <label className="flex items-center gap-1.5"><input type="checkbox" checked={item.par} onChange={(e) => handleItemChange(index, 'par', e.target.checked)} className="w-3.5 h-3.5 rounded text-red-600 focus:ring-0 border-gray-300"/> PAR</label>
                                                            <label className="flex items-center gap-1.5"><input type="checkbox" checked={item.ics} onChange={(e) => handleItemChange(index, 'ics', e.target.checked)} className="w-3.5 h-3.5 rounded text-red-600 focus:ring-0 border-gray-300"/> ICS</label>
                                                            <label className="flex items-center gap-1.5"><input type="checkbox" checked={item.iar} onChange={(e) => handleItemChange(index, 'iar', e.target.checked)} className="w-3.5 h-3.5 rounded text-red-600 focus:ring-0 border-gray-300"/> IAR</label>
                                                        </div>
                                                    </td>
                                        ...
                                        <tfoot className="bg-gray-50 border-t border-gray-200">
                                            <tr>
                                                <td colSpan={5} className="px-4 py-3 text-right text-sm font-bold text-gray-600">Total Amount</td>
                                                <td className="px-4 py-3 text-right text-sm font-bold text-red-900">₱{form.data.total.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                                                <td colSpan={2}></td>
                                            </tr>
                                        </tfoot>
                                    </table>
