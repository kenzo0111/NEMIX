...
export default function InventoryFormModal({
    ...
    // Automatically update SKU preview in Add mode
    useEffect(() => {
        if (!isEditing && !data.sku) {
            if (data.supplier_id && suppliers.length > 0) {
                ...
                const clean = data.name.replace(/[^a-zA-Z0-9]/g, '').slice(0, 4).toUpperCase();
                const padCount = String(existingItems.length + 1).padStart(4, '0');
                setData('sku', `ITEM-${clean}-${padCount}`);
            }
        }
    }, [data.supplier_id, data.name, isEditing, existingItems, suppliers]);
    ...
    return (
        <Modal show={show} onClose={onClose} maxWidth="2xl" closeable={!processing}>
            <div className="relative bg-white rounded-lg shadow-xl overflow-hidden border border-gray-200">
                ...
                {potentialDuplicate && (
                    <div className="mx-6 mt-4 p-3.5 bg-amber-50 border border-amber-300 rounded-lg text-amber-900 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                        <div className="text-xs">
                            <p className="font-bold flex items-center gap-1.5 text-amber-950">
                                <span>⚠️</span> Potential Duplicate Item Master Detected
                            </p>
                            <p className="mt-0.5 text-amber-800">
                                An item named <span className="font-semibold text-gray-900 font-mono">"{potentialDuplicate.name}"</span> ({potentialDuplicate.sku || 'No SKU'}) is already registered. If receiving stock from a new supplier or at a different cost, record a <strong>Receiving</strong> instead of creating a duplicate item.
                            </p>
                        </div>
                ...
                <form onSubmit={onSubmit}>
                    <div className="p-6 space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="e.g. Bond Paper A4 80 GSM"
                                    required
                                    disabled={processing}
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-[11px] text-gray-500">
                                    Unique institutional catalog identifier.
                                </p>
                                {errors.sku && (
                        ...
                        {/* Group 2: Unit of Issue + Stock Status */}
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-semibold text-gray-700 mb-1">
                            ...
                            <div>
                                ...
                                />
                                <p className="mt-1 text-[11px] text-gray-500">
                                    Computed automatically from on-hand batches (low stock threshold: {lowStockThreshold} units).
                                </p>
                            </div>
                        </div>
