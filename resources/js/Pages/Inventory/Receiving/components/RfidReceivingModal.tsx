...
export const RfidReceivingModal: React.FC<Props> = ({ show, onClose, scanInput, onScanInputChange, onScanLookup, scannedItems, onRemove, suppliers, supplierIds, costs, onCostChange, connectionState, scanFeedback, onSupplierChange, dateReceived, onDateChange, isSearching, processing, errorMessage, submitError, onSubmit }) => {
    const inputRef = useRef<HTMLInputElement>(null);
    useEffect(() => { if (show) inputRef.current?.focus(); }, [show]);
    const ready = scannedItems.length > 0 && scannedItems.every(item => {
        const cost = costs[item.rfid_tag || ''];
        return Boolean(supplierIds[item.rfid_tag || '']) && cost !== undefined && cost !== '' && Number.isFinite(Number(cost)) && Number(cost) >= 0 && /^\d+(\.\d{1,2})?$/.test(cost);
        ...
        <Modal show={show} onClose={onClose} maxWidth="lg" ariaLabel="Scan RFID Tags">
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
                ...
                <div className="px-5 py-4 border-b border-gray-200 flex items-center justify-between bg-gray-50">
                    <div className="flex items-center gap-3"><Radio className="w-5 h-5 text-red-900" /><div><h3 className="text-base font-bold font-serif">Scan RFID</h3><p className="text-xs text-gray-500">Each tag identifies an item type. Review one receipt line per tag; this does not track individual physical units.</p></div></div>
                ...
                <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
                    <p role="status" className="text-xs text-gray-700">Hardware feed API: {connectionState}. {scanFeedback} Keyboard and manual entry are available.</p>
                    ...
                    <div className="flex items-center justify-between"><h4 className="text-sm font-bold">Scanned items ({scannedItems.length})</h4><span className="text-xs text-gray-500">One receipt line per tag</span></div>
                    {scannedItems.length === 0 ? <p className="text-sm text-gray-500 border border-dashed border-gray-300 rounded-lg p-5 text-center">No tags scanned yet.</p> : (
                        <div className="space-y-2">{scannedItems.map(item => {
                            ...
                            return <div key={tag} className="p-3 border border-gray-200 rounded-lg flex flex-col sm:flex-row sm:items-center gap-3">
                                ...
                                <label className="text-xs text-gray-700">Supplier <select value={supplierIds[tag] ?? ''} onChange={event => { onSupplierChange(tag, event.target.value ? Number(event.target.value) : ''); inputRef.current?.focus(); }} disabled={processing} className="block mt-1 rounded-md border-gray-300 text-xs w-full sm:w-44"><option value="">Select supplier</option>{suppliers.map(supplier => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</select></label>
                                <label className="text-xs text-gray-700">Actual unit cost <input type="number" min="0" max="9999999999.99" step="0.01" value={costs[tag] ?? ''} onChange={event => onCostChange(tag, event.target.value)} onBlur={event => { if (!event.relatedTarget) inputRef.current?.focus(); }} disabled={processing} className="block mt-1 rounded-md border-gray-300 text-xs w-full sm:w-36" /></label>
                    ...
                    <label className="block text-xs font-semibold">Date received <input type="date" value={dateReceived} onChange={event => { onDateChange(event.target.value); inputRef.current?.focus(); }} disabled={processing} className="block mt-1 rounded-md border-gray-300 text-sm" /></label>
                </div>
