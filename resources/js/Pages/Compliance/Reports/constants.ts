...
export const customSelectStyles = {
    control: (provided: any, state: any) => ({
        ...
        borderColor: state.isDisabled ? 'var(--border-color, #e5e7eb)' : (state.isFocused ? '#7f1d1d' : 'var(--border-color, #d1d5db)'),
        borderWidth: '1px',
        padding: '1px 2px',
        minWidth: '150px',
        ...
        fontSize: '0.8125rem',
        fontWeight: '500',
        backgroundColor: state.isDisabled ? 'rgba(120, 120, 120, 0.1)' : 'var(--surface-card, #ffffff)',
        color: state.isDisabled ? 'var(--text-secondary, #4b5563)' : 'var(--text-primary, #111827)',
        cursor: state.isDisabled ? 'not-allowed' : 'default',
        opacity: state.isDisabled ? 0.9 : 1,
        '&:hover': { borderColor: state.isDisabled ? 'var(--border-color, #e5e7eb)' : '#7f1d1d' },
    }),
    option: (provided: any, state: any) => ({
        ...provided,
        backgroundColor: state.isSelected ? '#7f1d1d' : state.isFocused ? 'rgba(153, 27, 27, 0.2)' : 'var(--surface-card, #ffffff)',
    ...
    singleValue: (provided: any, state: any) => ({
        ...provided,
        color: state.isDisabled ? 'var(--text-secondary, #374151)' : 'var(--text-primary, #111827)',
        fontWeight: '500',
    }),
    input: (provided: any) => ({
        ...provided,
